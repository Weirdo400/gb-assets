...
export default function AdminWithdrawals() {
  const [withdrawals, setWithdrawals] = useState<(Transaction & { userName?: string })[]>([]);
  ...
  const approve = async (tx: Transaction & { userName?: string }) => {
    ...
    const newBal = Math.max(0, current - tx.amount);
    await updateDoc(doc(db, "users", tx.uid), { availableBalance: newBal });
    toast.success(`Approved withdrawal of ${fmtCurrency(tx.amount)}`);
    await load();
  };
  ...
  const filtered = withdrawals.filter(t => filter === "all" || t.status === filter);

  return (
    <div>
      <div className="px-10 py-8 border-b border-border flex items-center justify-between">
        <div>
          <div className="metric-label mb-1">Withdrawals</div>
      ...
      <div className="gb-row-header grid" style={{ gridTemplateColumns: "140px 1fr 120px 100px 1fr 140px" }}>
        <div>Date</div>
        <div>User</div>
      ...
      {filtered.map(tx => (
        <div key={tx.id} className="gb-row grid items-center" style={{ gridTemplateColumns: "140px 1fr 120px 100px 1fr 140px", cursor: "default" }}>
